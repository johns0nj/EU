...
# 创建图表
fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(24, 13.5))  # 16:9比例 (24/13.5=1.777)
...
bars1 = ax1.barh(benchmark_data['国家'], benchmark_data['MSCI AC World权重'], 
                 color=[colors[country] for country in benchmark_data['国家']])  # 使用颜色方案
...
# 2. TAA组合权重图
taa_data = df.set_index('国家').loc[country_order].reset_index()
bars2 = ax2.barh(taa_data['国家'], taa_data['TAA组合国家权重'], 
                 color=[colors[country] for country in taa_data['国家']])  # 使用颜色方案
ax2.set_title('境外TAA组合权重建议 (%)', pad=20, fontsize=32)
# 添加数值标签
for bar in bars2:
    width = bar.get_width()
...
diff_data = df.set_index('国家').loc[country_order].reset_index()
bars3 = ax3.barh(diff_data['国家'], diff_data['差异'], 
                 color=['#FF4B4B' if x < 0 else '#4CAF50' for x in diff_data['差异']])
ax3.set_title('权重差异（境外TAA组合 - MSCI AC World） (%)', pad=20, fontsize=32)
# 添加数值标签
for bar in bars3:
    width = bar.get_width()
...
# 设置x轴范围和标签字体大小
max_weight = max(df['MSCI AC World权重'].max(), df['TAA组合国家权重'].max())
...
# 保存图表
plt.savefig('TAA_analysis.png', dpi=300, bbox_inches='tight')
print("图表已保存为 TAA_analysis.png")
