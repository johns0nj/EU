...
# 全球权重气泡图
ax1.scatter(global_df['国家'], [1] * len(global_df), s=global_df['权重（%）'] * 100, alpha=0.6)
ax1.set_title('全球权重分布', fontsize=24)
ax1.set_xlabel('国家', fontsize=20)
ax1.set_ylabel('', fontsize=20)
ax1.tick_params(axis='x', labelsize=16)
ax1.grid(True)

# 欧洲权重气泡图
ax2.scatter(europe_df['国家'], [1] * len(europe_df), s=europe_df['权重（%）'] * 1000, alpha=0.6)
ax2.set_title('欧洲权重分布', fontsize=24)
ax2.set_xlabel('国家', fontsize=20)
...
for attempt in range(max_retries):
    try:
        # 使用 openpyxl 引擎
        with pd.ExcelWriter('MSCIACW_with_bubbles.xlsx', engine='openpyxl') as writer:
            global_df.to_excel(writer, sheet_name='全球权重', index=False)
            europe_df.to_excel(writer, sheet_name='欧洲权重', index=False)
            
            # 将图表保存为图片
            plt.savefig('bubbles.png', dpi=300, bbox_inches='tight')
...
print("数据和图表已成功保存至 MSCIACW_with_bubbles.xlsx")
