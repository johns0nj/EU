...
# 欧洲权重环形图
wedges, texts, autotexts = ax2.pie(europe_df['权重（%）'], 
                                   labels=europe_df['国家'], 
                                   autopct='%1.1f%%',
                                   startangle=90,
                                   pctdistance=0.85,
...
for attempt in range(max_retries):
    try:
        # 使用 openpyxl 引擎
        with pd.ExcelWriter('MSCIACW_with_donuts.xlsx', engine='openpyxl') as writer:
            global_df.to_excel(writer, sheet_name='全球权重', index=False)
            europe_df.to_excel(writer, sheet_name='欧洲权重', index=False)
            
            # 将图表保存为图片
            plt.savefig('donuts.png', dpi=300, bbox_inches='tight')
...
print("数据和图表已成功保存至 MSCIACW_with_donuts.xlsx")
